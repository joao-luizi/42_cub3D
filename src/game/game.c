...
static inline void	rotate_player(t_app_state *state,
		double rotation_speed)
{
	...
	state->player.direction.x = state->player.direction.x
		* cos(rotation_speed) - state->player.direction.y
		* sin(rotation_speed);
	...
	state->player.plane.x = state->player.plane.x
		* cos(rotation_speed) - state->player.plane.y
		* sin(rotation_speed);
	state->player.plane.y = old_plane_x * sin(rotation_speed)
		+ state->player.plane.y * cos(rotation_speed);
}

static inline void	move_player(double v_x, double v_y,
	t_app_state *state)
{
double	new_x;
double	new_y;
...
static inline void	vertical_move(t_app_state *state, double *v_x,
