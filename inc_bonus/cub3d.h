...
//		render_horizontal
void				render_ceiling_and_floor(t_app_state *st,
...
//		tex_utils
void				get_obs_tex(t_app_state *st, t_ray_info *r_info,
						t_obstacle *obs, int side);
int					get_pixel_color(t_img *img, int tex_x,
						int tex_y, float blending_factor);
...
bool				load_animation_frames(t_app_state *st,
						t_anim **anim, char *anim_file);
//		setup_map_utils
bool				validate_map(t_config *cfg);
//		setup_map
bool				isvalid_border(t_config *cfg, int i, int j);
void				set_player(int i, int j, t_config *cfg);
bool				calculate_map_dimensions(t_config *cfg,
						char **file_contents, size_t *index, size_t line_count);
